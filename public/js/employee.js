...
function verifiedLearning(data) {
  const completion = data.completion;
  if (!completion) return data;
  const modules = completion.modules.map((m) => {
    const raw = data.progress.modules.find((r) => r.key === m.key);
    if (m.verified)
      return {
        ...raw,
        ...m,
        source: "assessment",
        bestAttemptId: m.attemptId,
        status: m.passed ? "passed" : "retake",
      };
    if (raw?.source === "demo")
      return { ...raw, status: "not-started", classification: "Sample" };
    return {
      ...raw,
      score: null,
      status: "not-started",
  ...
  const status = completion.eligible
    ? "complete"
    : completion.modules.some((m) => m.verified && !m.passed)
      ? "retake"
      : completion.modules.some((m) => m.verified || m.activeAttemptId)
        ? "in-progress"
        : "not-started";
  return {
    ...data,
    progress: {
      ...
      status,
      hasDemoScores: modules.some((m) => m.source === "demo"),
    },
  };
}
...
function progress(ctx, data) {
  ...
  return {
    title: "My progress",
    html: `<div class="page-heading"><div><h1>Small steps. Safer habits.</h1><p>Your progress across the three required safety modules.</p></div>${progressBadge(p.status)}</div><a href="#/employee/certificates" class="btn btn-secondary completion-summary-link">${icon("award", 15)}Completion summary & certificate</a><section class="panel learning-progress-card" style="margin:0 0 24px"><div class="progress-ring" style="--value:${p.percent}"><span>${p.percent}%</span></div><div><h3>${p.passed} of 3 modules passed</h3><p>${p.overallScore === null ? "Your overall score will appear once all three modules are assessed." : `Overall score: ${p.overallScore} / 100. Every individual module still needs at least 70.`}</p></div></section>${p.hasDemoScores ? `<div class="scope-note">${icon("info", 16)}<span><strong>Sample progress.</strong> These demonstration scores illustrate the reporting interface. They are not evidence of completed safety training.</span></div>` : ""}<section class="panel"><header class="panel-head"><div><h2>Module-by-module progress</h2><p>Your next safe decision starts with awareness.</p></div>${icon("chart", 18)}</header><div class="panel-body"><div class="detail-modules">${p.modules.map((m) => `<div class="detail-module"><span class="module-small-icon">${icon(moduleMeta[m.key].icon)}</span><div><strong>${moduleMeta[m.key].short}</strong><small>${m.score === null ? "No assessment yet" : m.source === "demo" ? "Illustrative sample — not a completed assessment" : `${m.attempts} attempt${m.attempts === 1 ? "" : "s"} · ${formatDate(m.assessedAt)}`}</small></div><span style="margin-left:auto" class="score ${m.score === null ? "none" : m.score >= 85 ? "excellent" : m.score >= 70 ? "pass" : "fail"}">${m.score === null ? "—" : `${m.score}<small>/100</small>`}</span><span class="badge ${m.score === null ? "gray" : m.score >= 70 ? "green" : "amber"}">${m.classification}</span></div>`).join("")}</div><div class="info-callout mt-24">${icon("award", 17)}<span>Certificate eligibility requires <strong>70/100 or higher in all three modules</strong>. Below 70 means a retake. All three assessments are available now. Pass all three genuine assessments, then open Certificates to generate your completion PDF.</span></div></div></section><div class="progress-legend"><span><i></i>Excellent · 85–100</span><span><i class="blue"></i>Pass · 70–84</span><span><i class="amber"></i>Retake · below 70</span></div>`,
    mount() {},
  };
}
export async function employeePage(route, ctx) {
  ...
  if (routeName === "/employee/module/hazard-perception") {
    const { hazardPerceptionPage } = await import("./hazard-perception.js");
    return hazardPerceptionPage(ctx, attemptId);
  }
  if (route === "/employee/certificates") {
    const { certificatePage } = await import("./certificates.js");
    return certificatePage(ctx);
  }
  if (route === "/employee/settings") return settingsPage(ctx);
  const data = verifiedLearning(await ctx.api.get("/me/training"));
