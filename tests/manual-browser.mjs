// Full Manual Handling browser flow using a disposable MongoDB instance.
import { chromium, expect } from "@playwright/test";
import assert from "node:assert/strict";
import { mkdtemp, mkdir, rm } from "node:fs/promises";
import path from "node:path";
import os from "node:os";
const tmp = await mkdtemp(path.join(os.tmpdir(), "zi-manual-browser-"));
...
const artifacts = path.resolve("data/manual-browser-artifacts");
...
async function login(page, username = "manual.learner") {
...
try {
  ...
  await login(page);
  await expect(page.locator('[data-module="manual-handling"]')).toContainText(
    "Start training",
  );
  await expect(page.locator('[data-module="hazard-perception"]')).toContainText(
    "View module overview",
  );
  await page.locator('[data-module="manual-handling"]').click();
  ...
  await page.screenshot({
    path: path.join(artifacts, "manual-intro.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  await expect
    ...
    .toBe("rgb(25, 26, 30)");
  await page.screenshot({
    path: path.join(artifacts, "manual-activity-dark.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  await page.screenshot({
    path: path.join(artifacts, "manual-timed-quiz.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  await page.screenshot({
    path: path.join(artifacts, "manual-result.png"),
    fullPage: true,
    animations: "disabled",
  });
  await page.reload();
  ...
  await page
    ...
    .click();
  await expect(page.locator(".detail-module").first()).toContainText("94");
  ...
  for (const [index, task] of course.tasks.entries()) {
    ...
    if (index === 1)
      await page.screenshot({
        path: path.join(artifacts, "manual-mobile.png"),
        fullPage: true,
        animations: "disabled",
      });
  ...
  await accessible.locator('[data-module="manual-handling"]').click();
  ...
  console.log(
    "ALL MANUAL HANDLING BROWSER CHECKS PASSED. No uncaught browser errors.",
  );
} catch (e) {
  let index = 0;
